...
type ManagerTab = ReferentielCategorie | 'nature_compte';
...
export const ReferentielsManager = () => {
  ...
  const referentielCategorie = activeCategorie === 'nature_compte' ? null : activeCategorie;
    ...
    try {
      if (activeCategorie === 'nature_compte') {
        if (selectedNatureCompte) {
          ...
          toast({
            title: 'Nature de compte mise à jour',
            description: 'La nature de compte a été mise à jour avec succès.',
          });
        } else {
          await naturesCompteService.create({
            ...
            compteDefautId:
              typeof data.compteDefautId === 'string' && data.compteDefautId.length > 0
                ? data.compteDefautId
                : undefined,
          ...
          toast({
            title: 'Nature de compte créée',
            description: 'La nature de compte a été créée avec succès.',
          });
        }
  ...
  const handleDelete = async () => {
    try {
      if (activeCategorie === 'nature_compte') {
        ...
        toast({
          title: 'Nature de compte supprimée',
          description: 'La nature de compte a été supprimée avec succès.',
        });
        queryClient.invalidateQueries({ queryKey: ['natures-compte'] });
      } else {
                        ...
                        {isLoadingNatures ? (
                          ...
                          <TableRow>
                            <TableCell colSpan={7} className="text-center text-muted-foreground">
                              Aucune nature de compte renseignée
                            </TableCell>
                          </TableRow>
                        ) : (
                          naturesCompte.map((natureCompte) => (
                            <TableRow key={natureCompte.id}>
                              ...
                              <TableCell className="text-sm">
                                {natureCompte.compteDefaut
                                  ? `${natureCompte.compteDefaut.numero} - ${natureCompte.compteDefaut.libelle}`
                                  : '-'}
                              </TableCell>
                              <TableCell className="max-w-xs truncate text-sm text-muted-foreground">
                                {natureCompte.description || '-'}
                              </TableCell>
                              ...
                              <TableCell className="text-right">
                                <div className="flex justify-end gap-2">
                                  ...
                                  <Button
                                    variant="ghost"
                                    size="sm"
                                    onClick={() => handleDeleteReferentielClick(referentiel)}
                                    disabled={!referentiel.modifiable}
                                  >
                                    <Trash2 className="h-4 w-4" />
                                  </Button>
                                </div>
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Êtes-vous sûr de vouloir supprimer {activeCategorie === 'nature_compte' ? 'la nature de compte' : 'le référentiel'} "{deleteLabel}" ?
