...
const AppLayout = () => {
  ...
  return <div className="min-h-screen bg-background flex w-full">
      ...
      <aside className={`${sidebarOpen ? 'w-64' : 'w-16'} bg-card border-r border-border transition-all duration-300 flex flex-col`}>
        ...
        {/* Sélecteurs */}
        {sidebarOpen && <div className="p-4 space-y-3 border-b border-border">
            {/* Sélecteur de client */}
            <div>
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
          {navigationSections.map((section, sectionIndex) => <Collapsible key={section.title} open={openSections[section.title]} onOpenChange={() => toggleSection(section.title)} className="space-y-1">
              ...
              {sidebarOpen && sectionIndex < navigationSections.length - 1 && <Separator className="my-2" />}
            </Collapsible>)}
        </nav>
        ...
        <div className="p-4 border-t border-border">
          {sidebarOpen ? <div className="flex items-center gap-3">
            ...
            </div> : <Button variant="ghost" size="icon" onClick={handleLogout} className="w-full">
              <LogOut className="h-4 w-4" />
            </Button>}
        </div>
      </aside>
