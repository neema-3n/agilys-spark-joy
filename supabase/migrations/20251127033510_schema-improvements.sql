...
ALTER TABLE engagements DROP CONSTRAINT IF EXISTS engagements_client_exercice_numero_key;
...
ALTER TABLE factures DROP CONSTRAINT IF EXISTS factures_client_exercice_numero_key;
...
ALTER TABLE bons_commande DROP CONSTRAINT IF EXISTS bons_commande_client_exercice_numero_key;
...
ALTER TABLE paiements DROP CONSTRAINT IF EXISTS paiements_client_exercice_numero_key;
...
ALTER TABLE recettes DROP CONSTRAINT IF EXISTS recettes_client_exercice_numero_key;
...
ALTER TABLE operations_tresorerie DROP CONSTRAINT IF EXISTS operations_tresorerie_client_exercice_numero_key;
...
ALTER TABLE reservations_credits DROP CONSTRAINT IF EXISTS reservations_credits_client_exercice_numero_key;
...
ALTER TABLE modifications_budgetaires DROP CONSTRAINT IF EXISTS modifications_budgetaires_client_exercice_numero_key;
...
-- Depenses
ALTER TABLE depenses DROP CONSTRAINT IF EXISTS depenses_statut_check;
ALTER TABLE depenses ADD CONSTRAINT depenses_statut_check 
  CHECK (statut IN ('brouillon', 'validee', 'ordonnancee', 'payee', 'annulee'));

-- Engagements
ALTER TABLE engagements DROP CONSTRAINT IF EXISTS engagements_statut_check;
...
ALTER TABLE reservations_credits DROP CONSTRAINT IF EXISTS reservations_credits_statut_check;
...
ALTER TABLE operations_tresorerie DROP CONSTRAINT IF EXISTS operations_tresorerie_statut_check;
...
ALTER TABLE modifications_budgetaires DROP CONSTRAINT IF EXISTS modifications_budgetaires_statut_check;
ALTER TABLE modifications_budgetaires ADD CONSTRAINT modifications_budgetaires_statut_check 
  CHECK (statut IN ('brouillon', 'en_attente', 'validee', 'rejetee', 'annulee'));

-- Recettes
ALTER TABLE recettes DROP CONSTRAINT IF EXISTS recettes_statut_check;
...
ALTER TABLE engagements DROP CONSTRAINT IF EXISTS engagements_montant_check;
...
ALTER TABLE factures DROP CONSTRAINT IF EXISTS factures_montant_ht_check;
ALTER TABLE factures ADD CONSTRAINT factures_montant_ht_check 
  CHECK (montant_ht >= 0);

ALTER TABLE factures DROP CONSTRAINT IF EXISTS factures_montant_ttc_check;
ALTER TABLE factures ADD CONSTRAINT factures_montant_ttc_check 
  CHECK (montant_ttc >= 0);

ALTER TABLE bons_commande DROP CONSTRAINT IF EXISTS bons_commande_montant_check;
ALTER TABLE bons_commande ADD CONSTRAINT bons_commande_montant_check 
  CHECK (montant >= 0);

ALTER TABLE reservations_credits DROP CONSTRAINT IF EXISTS reservations_credits_montant_check;
...
ALTER TABLE operations_tresorerie DROP CONSTRAINT IF EXISTS operations_tresorerie_montant_check;
ALTER TABLE operations_tresorerie ADD CONSTRAINT operations_tresorerie_montant_check 
  CHECK (montant >= 0);

ALTER TABLE recettes DROP CONSTRAINT IF EXISTS recettes_montant_check;
ALTER TABLE recettes ADD CONSTRAINT recettes_montant_check 
  CHECK (montant >= 0);

ALTER TABLE modifications_budgetaires DROP CONSTRAINT IF EXISTS modifications_budgetaires_montant_check;
...
DROP TRIGGER IF EXISTS update_bons_commande_updated_at ON bons_commande;
CREATE TRIGGER update_bons_commande_updated_at 
  BEFORE UPDATE ON bons_commande 
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_reservations_credits_updated_at ON reservations_credits;
CREATE TRIGGER update_reservations_credits_updated_at 
  BEFORE UPDATE ON reservations_credits 
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_modifications_budgetaires_updated_at ON modifications_budgetaires;
CREATE TRIGGER update_modifications_budgetaires_updated_at 
  BEFORE UPDATE ON modifications_budgetaires 
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_operations_tresorerie_updated_at ON operations_tresorerie;
...
ALTER TABLE modifications_budgetaires DROP CONSTRAINT IF EXISTS modifications_budgetaires_ligne_source_id_fkey;
...
ALTER TABLE modifications_budgetaires DROP CONSTRAINT IF EXISTS modifications_budgetaires_ligne_destination_id_fkey;
ALTER TABLE modifications_budgetaires 
...
ALTER TABLE engagements DROP CONSTRAINT IF EXISTS engagements_dates_check;
...
ALTER TABLE depenses DROP CONSTRAINT IF EXISTS depenses_dates_check;
...
ALTER TABLE depenses DROP CONSTRAINT IF EXISTS depenses_ordonnancement_dates_check;
...
ALTER TABLE depenses DROP CONSTRAINT IF EXISTS depenses_paiement_dates_check;
