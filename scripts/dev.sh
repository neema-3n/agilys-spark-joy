...
INITIAL_DB_PORT="${DB_PORT-}"
INITIAL_APP_ENV="${APP_ENV-}"
INITIAL_NEXT_PUBLIC_APP_ENV="${NEXT_PUBLIC_APP_ENV-}"
INITIAL_POSTGRES_DB="${POSTGRES_DB-}"
INITIAL_POSTGRES_USER="${POSTGRES_USER-}"
INITIAL_POSTGRES_PASSWORD="${POSTGRES_PASSWORD-}"
INITIAL_NEXT_PUBLIC_API_BASE_URL="${NEXT_PUBLIC_API_BASE_URL-}"
INITIAL_NEXT_PUBLIC_SITE_URL="${NEXT_PUBLIC_SITE_URL-}"
INITIAL_JWT_ACCESS_SECRET="${JWT_ACCESS_SECRET-}"
INITIAL_JWT_REFRESH_SECRET="${JWT_REFRESH_SECRET-}"
INITIAL_JWT_ACCESS_TTL_SECONDS="${JWT_ACCESS_TTL_SECONDS-}"
INITIAL_JWT_REFRESH_TTL_SECONDS="${JWT_REFRESH_TTL_SECONDS-}"
...
WEB_PORT="${INITIAL_WEB_PORT:-${WEB_PORT:-8080}}"
API_PORT="${INITIAL_API_PORT:-${API_PORT:-3001}}"
DB_PORT="${INITIAL_DB_PORT:-${DB_PORT:-${POSTGRES_PORT:-5432}}}"
APP_ENV="${INITIAL_APP_ENV:-${APP_ENV:-development}}"
NEXT_PUBLIC_APP_ENV="${INITIAL_NEXT_PUBLIC_APP_ENV:-${NEXT_PUBLIC_APP_ENV:-${APP_ENV}}}"
POSTGRES_DB="${INITIAL_POSTGRES_DB:-${POSTGRES_DB:-agilys}}"
POSTGRES_USER="${INITIAL_POSTGRES_USER:-${POSTGRES_USER:-agilys_app}}"
POSTGRES_PASSWORD="${INITIAL_POSTGRES_PASSWORD:-${POSTGRES_PASSWORD:-change-me-local-only}}"
if [[ -n "${INITIAL_NEXT_PUBLIC_API_BASE_URL}" ]]; then
  API_BASE_URL="${INITIAL_NEXT_PUBLIC_API_BASE_URL}"
elif [[ -n "${INITIAL_API_PORT}" ]]; then
  API_BASE_URL="http://localhost:${API_PORT}"
else
  API_BASE_URL="${NEXT_PUBLIC_API_BASE_URL:-http://localhost:${API_PORT}}"
fi

if [[ -n "${INITIAL_NEXT_PUBLIC_SITE_URL}" ]]; then
  SITE_URL="${INITIAL_NEXT_PUBLIC_SITE_URL}"
elif [[ -n "${INITIAL_WEB_PORT}" ]]; then
  SITE_URL="http://localhost:${WEB_PORT}"
else
  SITE_URL="${NEXT_PUBLIC_SITE_URL:-http://localhost:${WEB_PORT}}"
fi
JWT_ACCESS_SECRET="${INITIAL_JWT_ACCESS_SECRET:-${JWT_ACCESS_SECRET:-dev-access-secret}}"
JWT_REFRESH_SECRET="${INITIAL_JWT_REFRESH_SECRET:-${JWT_REFRESH_SECRET:-dev-refresh-secret}}"
JWT_ACCESS_TTL_SECONDS="${INITIAL_JWT_ACCESS_TTL_SECONDS:-${JWT_ACCESS_TTL_SECONDS:-900}}"
JWT_REFRESH_TTL_SECONDS="${INITIAL_JWT_REFRESH_TTL_SECONDS:-${JWT_REFRESH_TTL_SECONDS:-604800}}"
